...
|  USAGE: 
---------
|  rsh [ops] $1 [VERSION_TYPE] [CUSTOM_VERSION]
|
|  VERSION_TYPE:           Increment type to use
|                            Options are: 0.0.0.0 
|                             - Release   1.0.0.0
|                             - Beta      0.1.0.0
|                             - Alpha     0.0.1.0
|                             - Dev       0.0.0.1
|                             - Custom    Uses CUSTOM_VERSION
|  CUSTOM_VERSION:         Used when VERSION_TYPE is \`custom\`
-----------
EOF
    :;;
  push|send) 
...
|  rsh [ops] $1
|
|  Push $repo_name to $push_path; takes no ARGs
-----------
EOF
:;;
  pull|yank) 
...
EOF
  ...
  init) :;;
  # TODO:
  # remove everything about init since its useless
  config) 
    shcat << EOF
---------
|  USAGE:
---------
|  rsh [ops] $1 <VARIABLE> <NEW_VALUE>
|
|  VARIABLE:    Variable within ./${config_hidden}rsh.conf to be changed.
|  NEW_VALUE:   Value to change above variable to.
-----------
EOF
:;;
    # TODO:
...
[ "$pull_path" ] && {
  ...
  # now basically wipe out any returning files that are present in the db
